## To run the program use the below comments in the terminal(crtl+`)
## cd Documents/Github/umfb_streamlit
## streamlit run main_osu_demo.py
...
from functions_osu import get_player_force_plate_data, get_position_force_plate_data, get_catapult_data, get_body_weight_data, get_internal_load_data, get_player_perch_data, get_position_perch_data, get_max_for_exercise, get_hydration_data, filter_dates, get_percentile_data

today = datetime.date.today()
st.set_page_config(initial_sidebar_state="expanded", page_icon='https://a.espncdn.com/combiner/i?img=/i/teamlogos/ncaa/500/194.png&h=200&w=200', page_title='Sport Science Demo Player Dashboard')
...
player_df = pd.read_csv('hashed_credentials osu.csv')
...
authenticator = stauth.Authenticate(credentials, 'player_dashboard', 'player_dashboard_key', cookie_expiry_days=1, auto_hash=False)
...
if st.session_state.get('authentication_status'):
    ...
    ## New
    player_df['app_username'] = player_df['app_username'].str.lower()
    ...
    player_peak_cmj_jump, player_peak_cmj_date, cmj_df = get_player_force_plate_data(playerid)
    ## Position Force Plate Data (2025)
    raw_position_cmj_df, position_cmj_df_line, position_name, position_cmj_df = get_position_force_plate_data(positionid)
    ...
    ## Catapult Data
    catapult_df = get_catapult_data(playerid)
    ## Body Weight Data
    body_weight_df = get_body_weight_data(playerid)
    ## Internal Load Data
    internal_df = get_internal_load_data(playerid)
    ## Perch Data
    player_perch_df = get_player_perch_data(playerid)
    ## Position Perch Data (2025)
    position_perch_df = get_position_perch_data(positionid)
    ## Hydration Data
    hydration_df = get_hydration_data(playerid)
    ...
    with col2:
        st.image('https://a.espncdn.com/combiner/i?img=/i/teamlogos/ncaa/500/194.png&h=200&w=200')
